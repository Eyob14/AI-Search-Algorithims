...
def eigenvector_centrality(graph):
    ...
    result = {k.value: v for k, v in zip(
        graph.graph.keys(), eigenvector_centrality)}
    return result
...
def katz_centrality(graph, alpha=0.1, beta=1.0):
    ...
    if alpha < 1/max_eigenvalue:
        ...
        katz_matrix = np.linalg.inv(
            np.eye(len_matrix)-alpha*adjacency_matrix)-np.eye(len_matrix)
...
def pageRank_centrality(graph, alpha=0.1, beta=1.0):
    ...
    eigenvalues, eigenvectors = np.linalg.eig(
        np.dot(adjacency_matrix, np.linalg.inv(Diag)))
    max_eigenvalue = max(eigenvalues)
    # in order for pageRank centrality to work this
    # is a condition that must be respected
    if alpha < 1/max_eigenvalue:
        pageRank_matrix = np.linalg.inv(
            np.eye(len_matrix)-alpha*np.dot(adjacency_matrix, np.linalg.inv(Diag)))
